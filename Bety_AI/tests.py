...
class HistorialConversacionTests(SimpleTestCase):
    ...
    @patch("Bety_AI.views.limpiar_pendiente_perfil_web")
    @patch("Bety_AI.views.consultar_qwen")
    @patch("Bety_AI.views.buscar_fragmentos_con_fallback")
    @patch("Bety_AI.views.guardar_interaccion_temporal", return_value=[])
    @patch("Bety_AI.views.interpretar_consulta_ia")
    def test_api_refuerza_busqueda_con_historial_si_depende_del_hilo(
        self,
        interpretar_mock,
        guardar_temporal_mock,
        buscar_mock,
        ...
        agregar_historial_conversacion(
            conversation_id,
            "como es el proceso de las ayudas economicas",
            "Las ayudas economicas tienen requisitos y documentos especificos.",
        )
        interpretar_mock.return_value = {
            "tipo_operacion": "consulta_documental",
            "consulta_normalizada": "requisitos para acceder a ayudas economicas del SGA UTEQ",
            "consulta_busqueda": "dame los requisitos que debo cumplir requisitos ayudas economicas documentos",
            "depende_historial": True,
            "formato_respuesta": "lista",
            "palabras_clave": ["ayudas economicas", "requisitos", "documentos"],
            "filtros_sugeridos": {},
            "modelo": "qwen-test",
        }
        buscar_mock.return_value = (
            [
                {
                    "contenido": "Para ayudas economicas, el estudiante debe cumplir requisitos y subir documentos.",
                    "metadata": {
                        "titulo": "Ayudas economicas",
                        "id_documento": "1",
                        "tipo_documento": "AYUDA_ECONOMICA",
        ...
        qwen_mock.return_value = {
            "respuesta": "Debes cumplir los requisitos de ayudas economicas y subir los documentos solicitados.",
            "modelo": "qwen-final",
        }
        ...
        request = APIRequestFactory().post(
            "/api/chat/",
            {
                "pregunta": "dame los requisitos que debo cumplir",
                "conversation_id": conversation_id,
                "usuario": {
                    "rol": "estudiante",
                    "tipo_estudio": "Pregrado",
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["tipo_respuesta"], "RESPUESTA")
        pregunta_busqueda = buscar_mock.call_args.kwargs["pregunta"]
        self.assertIn("como es el proceso de las ayudas economicas", pregunta_busqueda)
        self.assertIn("dame los requisitos que debo cumplir", pregunta_busqueda)
        self.assertIn("requisitos para acceder a ayudas economicas", pregunta_busqueda)
        guardar_temporal_mock.assert_called_once()
        limpiar_perfil_mock.assert_called_once()
